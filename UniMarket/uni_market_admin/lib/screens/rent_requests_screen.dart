...
class RentRequestsScreen extends StatefulWidget {
  ...
  @override
  State<RentRequestsScreen> createState() => _RentRequestsScreenState();
}
...
class _RentRequestsScreenState extends State<RentRequestsScreen> {
  ...
  void _showRentDetails(RentRequestModel request) {
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                _buildDetailRow('Request ID', request.rentRequestId),
                _buildDetailRow('Product ID', request.productId),
                _buildDetailRow('Product Title', request.productTitle),
                _buildDetailRow('Renter Name', request.renterName),
                _buildDetailRow('Start Date', request.startDate.toLocal().toString().split(' ')[0]),
                _buildDetailRow('End Date', request.endDate.toLocal().toString().split(' ')[0]),
                _buildDetailRow('Rental Days', '${request.rentalDays} Days'),
                _buildDetailRow('Price Per Day', '৳${request.pricePerDay.toStringAsFixed(2)}'),
                _buildDetailRow('Total Amount', '৳${request.totalAmount.toStringAsFixed(2)}'),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
            ...
            builder: (context, constraints) {
              ...
              final searchField = TextField(
                ...
                decoration: InputDecoration(
                  hintText: 'Search by renter name...',
                ...
                return Column(
                  ...
                  children: [
                    Text(
                      ...
                      style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold, color: const Color(0xFF1E3A5F)),
                    ),
              ...
              return Row(
                children: [
                  Expanded(
                    child: Text(
                      'Rent Requests Management',
                      style: GoogleFonts.outfit(fontSize: 22, fontWeight: FontWeight.bold, color: const Color(0xFF1E3A5F)),
                    ),
                  ),
          ...
          Expanded(
            child: StreamBuilder<List<RentRequestModel>>(
              stream: _rentService.getRentRequestsStream(),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return const Center(child: CircularProgressIndicator());
                }
                if (snapshot.hasError) {
                  return Center(child: Text('Error loading rent requests: ${snapshot.error}'));
                }

                var list = snapshot.data ?? [];
                if (_searchQuery.isNotEmpty) {
                  list = list.where((r) => r.renterName.toLowerCase().contains(_searchQuery)).toList();
                }

                if (list.isEmpty) {
                  return const Center(child: Text('No rent requests found.'));
                }
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: DataTable(
                        ...
                        columns: [
                          ...
                          DataColumn(label: Text('Renter', style: GoogleFonts.inter(fontWeight: FontWeight.bold))),
                          DataColumn(label: Text('Duration', style: GoogleFonts.inter(fontWeight: FontWeight.bold))),
                        ...
                        rows: list.map((request) {
                          return DataRow(
                            cells: [
                              ...
                              DataCell(Text(request.renterName)),
                              DataCell(Text('${request.rentalDays} Days')),
                              ...
                              DataCell(
                                IconButton(
                                  ...
                                  onPressed: () => _showRentDetails(request),
                                ),
                              ),
                            ],
                          );
